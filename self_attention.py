...
if __name__ == '__main__':
    # dff is number of units in dense feed forward layer
    # pe is max vocab size?
    sample_transformer = Transformer(num_layers=2, d_model=512, num_heads=2, dff=2048, vocab_size=324, pe_input=1000)
    input_test = tf.random.uniform((8, 512), dtype=tf.int64, minval=0, maxval=324)
    target_test = tf.random.uniform((8, 512), dtype=tf.int64, minval=0, maxval=324)
    la_mask, d_mask = create_masks(input_test)
    fn_out, _ = sample_transformer([input_test, False, la_mask, d_mask])
