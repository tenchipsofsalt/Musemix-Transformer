...
def steps2ticks(steps, bpm):
    return int(mido.second2tick(steps * 60 / (settings_keys.sampling_freq * bpm), settings_keys.ticks_per_beat,
                                mido.bpm2tempo(bpm)))
...
def decode(events, output_name, touhou=False):
    mid = mido.MidiFile(ticks_per_beat=settings_keys.ticks_per_beat)
    ...
    for i in range(event_count):
        event = events[i]
        if event <= settings_keys.tempo_offset:
            cur_vel = bin2vel(event)
        elif event <= settings_keys.note_offset:
            cur_tempo = (event - settings_keys.tempo_offset - 1) * settings_keys.tempo_step + settings_keys.min_tempo
            track.append(mido.MetaMessage('set_tempo', tempo=mido.bpm2tempo(cur_tempo), time=cur_pause))
            cur_pause = 0
        elif event <= settings_keys.pause_offset:
            if cur_tempo == 0:
                print("No tempo found, continuing...")
                continue
            note = event - settings_keys.note_offset - 1
            if note <= settings_keys.note_dim // 2:
                note_key = note // 128
                if cur_key != note_key:
                    cur_key = note_key
                    print(note_key)
                ...
                for j in range(i + 1, event_count):
                    if settings_keys.pause_offset < events[j] <= settings_keys.pedal_offset:
                        found_pause = True
                    elif events[j] == event + settings_keys.note_dim // 2:  # found note off
                        if found_pause:
                ...
                if note in cur_notes:
                    ...
                    cur_pause = 0
        elif event <= settings_keys.pedal_offset:
            if cur_tempo == 0:
                print("No tempo found, continuing...")
                continue
            cur_pause += steps2ticks(event - settings_keys.pause_offset, cur_tempo)
        else:
            value = event - settings_keys.pedal_offset
            if value == 3:
                # piano
...
if __name__ == '__main__':
    elise = np.load('Music/Albéniz/keyedEvents/Aragon (Fantasia) Op.47 part 6.mid.npy')
    decode(elise, 'keyed_test.mid')
