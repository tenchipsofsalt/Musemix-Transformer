...
class DataSequence(tf.keras.utils.Sequence):
    def __init__(self, dir_names, batch_size, seq_len, train=settings_keys.train_split, val=settings_keys.val_split, use_artist=True):
        self.files = []
        ...
        for dir_name in dir_names:
            count += 1
            for file in functions.get_files(f'Music/{dir_name}/keyedEvents/', '.npy'):
    ...
    def __getitem__(self, idx, source='train'):
        ...
        seqs = []
        for file in batch:
            seqs.append(note_shift(_get_seq(file, self.seq_len, self.use_artist), random.choice(settings_keys.shifts)))
        if self.use_artist:
...
class Dataset:
    def __init__(self, dir_path, train=settings_keys.train_split, val=settings_keys.val_split):
...
def _get_seq(file, seq_len, artist=True):
    ...
    if seq_len > data_length:
        ...
        if artist:
            ...
            seq = np.append([file[1] + settings_keys.artist_offset], seq)
        else:
...
def note_shift(events, shift):
    for i in range(len(events)):
        event = events[i]
        if settings_keys.note_offset < event <= settings_keys.pause_offset:
            key = (event - settings_keys.note_offset - 1) // 128
            note = (event - settings_keys.note_offset - 1) % 128
            note += shift
            if note < 0:
                note = 0
            elif note > 127:
                note = 127
            events[i] = settings_keys.note_offset + 1 + key * 128 + note
    return events